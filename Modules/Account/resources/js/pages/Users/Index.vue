  ...
  <div class="card pb-4">
    <div class="card-header align-items-center border-0 pt-5">
      ...
      <div class="d-flex align-items-center">
        ...
        <form class="input-group w-50" @submit.prevent="search()">
          <input
            v-model="searchQuery"
            type="search"
        ...
        <table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
          <thead>
            <tr class="fw-bolder text-muted">
              ...
              <th class="min-w-125px" @click="sort('name')">Имя</th>
              <th class="min-w-125px" @click="sort('email')">Почта</th>
              <th class="min-w-150px">Компания и ОКПО</th>
              <th class="min-w-150px">Локация</th>
              <th class="min-w-150px">Телефон и факс</th>
              <th class="min-w-125px">Роль</th>
              <th class="min-w-150px text-end">Действия</th>
            </tr>
          </thead>
          <tbody>
            <template v-for="(user, index) in paginator.collection" :key="index">
              <tr>
                <td>
                  <div class="d-flex align-items-center">
                    <div class="symbol symbol-50px me-5">
                      <span class="symbol-label bg-light">
                        <img :src="user.icon" class="w-100 align-self-end" />
                      </span>
                    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const search = () =>
      Inertia.get('', { searchQuery: searchQuery.value }, { preserveScroll: true })
