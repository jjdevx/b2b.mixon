    ...
    <form id="kt_account_profile_details_form" class="form" novalidate @submit.prevent="submit">
      <div class="card-body border-top p-9">
        ...
        <div class="fv-row mb-7">
          ...
          <select v-model="type" class="form-control form-control-lg form-control-solid">
            <option v-for="{ value, label } in typesForSelect" :key="value" :value="value">
              {{ label }}
            </option>
          </select>
          <div class="fv-plugins-message-container">
        ...
        <div class="fv-row mb-7">
          ...
          <Multiselect v-model="users" :options="usersForSelect" mode="tags" searchable />
          <div class="fv-plugins-message-container">
            <div class="fv-help-block">
              {{ errors.users }}
            </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const submit = handleSubmit((data: FormFields) => {
      ...
      Inertia.post(
        department.value
          ? route('departments.update', department.value.id)
